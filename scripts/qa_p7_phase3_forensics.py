...
# QA oracle only: these URLs prove the current source still has the product. They are
# never passed to production discovery/source memory and never become runtime seeds.
SOURCES = {
...
def main() -> int:
    ...
    report = {"input_identity": IDENTITY.model_dump(), "sources": {}}
    for label, (domain, oracle_url) in SOURCES.items():
        ...
        for row in signals:
            ...
            queries.append({
                ...
                "oracle_returned": oracle_url in urls,
        ...
        try:
            ...
            direct = {
                ...
                "exact_mpn_in_html": IDENTITY.mpn.casefold() in html.casefold(),
        ...
        report["sources"][label] = {
            ...
            "oracle_in_admitted_urls": oracle_url in admitted,
            "direct_pdp": direct,
        }
