...
def _parent_provenance(identity: ProductIdentity, candidate: SearchCandidate, anchor_text: str, rendered: bool) -> DocumentProvenance | None:
    ...
    try:
        ...
        authority = classify_source_authority(host, likely_official=bool(candidate.likely_official))
    except Exception:
        authority = "UNKNOWN"
    if str(authority).upper() != "MANUFACTURER":
        return None
    score = int(getattr(candidate, "identity_score", 0) or 0)
    confidence = max(.88, min(.99, score / 100 if score else .9))
    return DocumentProvenance(
        parent_url=candidate.url,
        parent_identity_status="EXACT" if score >= 88 else "STRONG",
        parent_identity_confidence=confidence,
        parent_authority="MANUFACTURER",
        anchor_text=anchor_text,
        discovery_method="rendered_landing_link" if rendered else "landing_link",
    )


def resolve_document_candidate_urls(identity: ProductIdentity, candidate: SearchCandidate, *, timeout: int = 12, trace=None) -> list[DocumentSearchCandidate]:
    url = str(candidate.url or "").strip()
    if not url:
        return []
    if _looks_like_direct_pdf(url):
        return [_resolved_candidate(candidate, url, candidate.title)]

    resolved: list[DocumentSearchCandidate] = []
    seen: set[str] = set()
    if trace:
        trace.emit("PDF_LANDING_INSPECTED", url=url, rendered=False)
    try:
    ...
    if page:
        for pdf_url in discover_pdf_candidates(url, page):
            if pdf_url in seen:
                continue
            seen.add(pdf_url)
            label = unquote(urlparse(pdf_url).path.rsplit("/", 1)[-1]) or "PDF"
            provenance = _parent_provenance(identity, candidate, label, False)
            if trace:
                trace.emit("PDF_LINK_FOUND", url=pdf_url, parent=url)
                if provenance:
                    trace.emit("PDF_PROVENANCE_BOUND", url=pdf_url, parent=url, authority=provenance.parent_authority)
            resolved.append(_resolved_candidate(candidate, pdf_url, label, provenance=provenance))

    try:
        browser_links = browser_pdf_links(url, timeout=max(timeout, 10), limit=10)
    except Exception:
        browser_links = []
    if browser_links and trace:
        trace.emit("PDF_LANDING_INSPECTED", url=url, rendered=True)
    for pdf_url, label in browser_links:
        if pdf_url in seen:
            continue
        seen.add(pdf_url)
...
def _resolve_valid_candidates(identity: ProductIdentity, candidates: list[SearchCandidate], *, limit: int, timeout: int, trace=None, landing_budget: list[int] | None = None, inspected_landings: set[str] | None = None) -> list[DocumentSearchCandidate]:
    resolved: list[DocumentSearchCandidate] = []
    ...
    for candidate in sorted(candidates, key=_document_rank, reverse=True):
        if not _looks_like_direct_pdf(candidate.url):
