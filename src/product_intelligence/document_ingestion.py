...
def process_pdf_document(
    ...
    *,
    ...
    """Download, protocol-validate, identity-validate and ingest one PDF.

    OCR/Mistral-related extraction can only happen after download_pdf has proved
    that the network response is an actual PDF. HTML/error bodies never reach
    ...
    match = validate_pdf_identity(identity, pdf_text, downloaded.final_url)
    if not match.accepted:
        if trace:
            trace.emit("PDF_DOWNLOAD_REJECTED", url=downloaded.final_url, reason=f"IDENTITY:{match.reason}")
    ...
    rec = build_record_strict(resolved_identity, evidence, [downloaded.final_url])
    rec.fetch = {
