...
def fill_excel_v8(template:str,output:str,records:list[ProductRecord],overwrite:bool=False,trace_path:str|None=None,ai_config:AIConfig|None=None,row_assignments:dict[tuple[str,int],ProductRecord]|None=None):
    ...
    for ws in wb.worksheets:
        ...
        for row in range(hr+1,last_row+1):
            ...
            for c,(key,mconf,field_class) in mapped.items():
                ...
                # AI may rewrite the description using multiple already-validated evidence rows.
                # This is the only field where AI gets priority over the deterministic single-source derivation.
                if ai.config.enabled and (ext=='53' or key_norm(_strip_field_id(header)) in {'descripcion','description'}) and rec.identity.match_level not in {'CONFLICT','LOW'}:
                    ...
                    if suggestion and float(suggestion.get('confidence') or 0)>=.84:
                        ...
                        ev_attr='; '.join(str(x.get('attribute','')) for x in evs[:6])
                        ev_raw='; '.join(str(x.get('value','')) for x in evs[:6])
                        ok,greason,_=validate_value(value,contracts[c],evidence_attribute=ev_attr,evidence_raw=ev_raw)
                        if ok:
                            cell.value=value
                            written.append({"sheet":ws.title,"cell":cell.coordinate,"header":header,"attribute":"ai_description","value":value,"source":"AI over validated evidence","reason":suggestion.get('reason'),"confidence":round(float(suggestion.get('confidence') or 0),3),"evidence_ids":suggestion.get('evidence_ids')})
                            continue
                ...
                # Seller fields/images never reach this layer; every result is validated again.
                if ai.config.enabled and rec.identity.match_level not in {'CONFLICT','LOW'}:
                    ...
                    if suggestion and float(suggestion.get('confidence') or 0)>=.82:
                        ...
                        if contracts[c].value_type=='controlled':
                            ...
                            value=cv
                        evs=suggestion.get('evidence') or []
                        ev_attr='; '.join(str(x.get('attribute','')) for x in evs[:4])
                        ev_raw='; '.join(str(x.get('value','')) for x in evs[:4])
                        ok,greason,_=validate_value(value,contracts[c],evidence_attribute=ev_attr,evidence_raw=ev_raw)
                        if ok:
                            cell.value=value
                            written.append({"sheet":ws.title,"cell":cell.coordinate,"header":header,"attribute":key or 'ai_derived',"value":value,"source":"AI over validated evidence","reason":suggestion.get('reason'),"confidence":round(float(suggestion.get('confidence') or 0),3),"evidence_ids":suggestion.get('evidence_ids')})
                            continue
