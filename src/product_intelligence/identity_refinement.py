...
def stable_model_core(value: str | None, *, raw: str | None = None, brand: str | None = None) -> str:
    ...
    for removable in (raw, brand):
        ...
        if token: text = re.sub(re.escape(token), " ", text, flags=re.I)
    tokens = re.findall(r"[A-Za-z0-9]+", text)
    core: list[str] = []
    for token in tokens:
        normalized = key_norm(token)
        if not normalized: continue
        if core and normalized in _MODEL_DESCRIPTOR_STOP: break
        if not core and normalized in _MODEL_DESCRIPTOR_STOP: continue
        core.append(token)
        if len(core) >= 5: break
    while core and key_norm(core[-1]) in _MODEL_DESCRIPTOR_STOP: core.pop()
    candidate = " ".join(core).strip()
    if len(core) < 2 or _compact(candidate) == _compact(raw): return ""
    return candidate

def _descriptive_title_signal(title: str) -> bool:
    useful = [t for t in _tokenize(title) if t not in _GENERIC and len(t) >= 2]
...
def _candidate_bound_to_raw(candidate: SearchCandidate, raw: str) -> bool:
    ...
    if target in _compact(candidate.title) or target in _compact(candidate.url): return True
...
def _title_tokens(candidate: SearchCandidate, raw: str) -> list[str]:
    ...
    if raw: text = re.sub(re.escape(raw), " ", text, flags=re.I)
    segments = [s.strip() for s in re.split(r"\s+[|–—]\s+", text) if s.strip()]
    if segments: text = max(segments, key=lambda s: sum(ch.isdigit() for ch in s) * 3 + len(s))
    return _tokenize(text)

def _url_tokens(candidate: SearchCandidate, raw: str) -> list[str]:
    path = unquote(urlparse(candidate.url).path or "")
    if raw: path = re.sub(re.escape(raw), " ", path, flags=re.I)
...
def _brand_ngrams(tokens: list[str]) -> list[str]:
    ...
    for length in (1,2,3):
        ...
        words=clean[:length]
        if any(any(ch.isdigit() for ch in w) for w in words): continue
        phrase=" ".join(words).strip()
        if len(_compact(phrase))>=2: out.append(phrase)
    return out

def _descriptive_brand_hint(value: str | None, raw: str) -> str | None:
    text=str(value or "").strip()
    if not text or _compact(text)==_compact(raw): return None
    for token in _tokenize(text)[:4]:
        if token in _BRAND_GENERIC or any(ch.isdigit() for ch in token): continue
...
def _select_brand(candidates:list[SearchCandidate],raw:str,current:str|None)->tuple[str|None,int,str|None]:
    domains_by_phrase:dict[str,set[str]]=defaultdict(set); display={}; host_matches=Counter(); current_key=_compact(current)
    for candidate in candidates:
        domain=_root(urlparse(candidate.url).hostname); root_label=_compact(domain.split(".",1)[0]); title_tokens=_title_tokens(candidate,raw)
        for phrase in _brand_ngrams(title_tokens):
            key=_compact(phrase); display.setdefault(key,phrase)
            if domain: domains_by_phrase[key].add(domain)
            if root_label and (key==root_label or (len(key)>=3 and key in root_label)): host_matches[key]+=1
        for token in title_tokens[:6]:
            if token in _BRAND_GENERIC or any(ch.isdigit() for ch in token) or len(token)<2: continue
            key=_compact(token)
            if not key: continue
            display.setdefault(key,token)
            if domain: domains_by_phrase[key].add(domain)
            if root_label and key==root_label: host_matches[key]+=1
    scores=[]
    for key,domains in domains_by_phrase.items():
        support=len(domains); words=display[key].split(); is_current=bool(current_key and (key==current_key or current_key.startswith(key) or key.startswith(current_key)))
        if support<2 and not is_current: continue
        score=support*10.0
        if is_current: score+=12.0
        if support>=2:
            score+=min(host_matches[key],support)*1.5
            if host_matches[key] and len(words)>1: score+=2.5
        score-=max(0,len(words)-1)*1.25
        scores.append((score,support,-len(words),key))
    if not scores: return current,0,None
    scores.sort(reverse=True); _,support,_,key=scores[0]; brand=display.get(key,current or key)
...
def _model_sequences(candidate:SearchCandidate,raw:str,brand:str|None)->list[list[str]]:
    sequences=[]; brand_tokens=set(_tokenize(brand or ""))
    for tokens in (_title_tokens(candidate,raw),_url_tokens(candidate,raw)):
        clean=[t for t in tokens if t not in brand_tokens and t not in _GENERIC]
        if clean: sequences.append(clean[:12])
    return sequences
...
def _select_model(candidates:list[SearchCandidate],raw:str,brand:str|None)->tuple[str|None,int]:
    ...
    for candidate in candidates:
        ...
        for sequence in _model_sequences(candidate,raw,brand):
            for phrase,start in _model_ngrams(sequence):
                key=_compact(phrase)
                if not key or key==_compact(raw): continue
                display.setdefault(key,phrase); earliest[key]=min(earliest.get(key,start),start)
                if domain: domains_by_ngram[key].add(domain)
    ranked=[]
    for key,domains in domains_by_ngram.items():
        support=len(domains)
        if support<2: continue
        phrase=display[key]; words=phrase.split()
        if words and all(key_norm(w) in _MODEL_DESCRIPTOR_STOP for w in words): continue
    ...
    ranked.sort(reverse=True); _,support,_,key=ranked[0]; return display[key],support

def _normalize_provider_candidate(row)->SearchCandidate|None:
    if isinstance(row,SearchCandidate): return row
...
def _plausible_brand_hint(value:str|None)->bool:
    tokens=_tokenize(value or ""); return bool(tokens and len(tokens)<=3 and all(t not in _BRAND_GENERIC for t in tokens))
...
def refine_code_identity(original:ProductIdentity,current:ProductIdentity,*,timeout:int=7,max_queries:int=2)->IdentityRefinement:
    raw=_raw(original)
    if not raw: return IdentityRefinement(current,None,0,0,0)
    explicit=str(current.brand or current.manufacturer or "").strip(); descriptive=_descriptive_brand_hint(current.model or current.product_name,raw); brand_hint=explicit or descriptive or ""
    ...
    for query in queries[:max(1,int(max_queries))]:
        for raw_candidate in _provider_search(query,max(5,min(int(timeout),8))):
            ...
            if candidate is None or candidate.url in seen or not _candidate_bound_to_raw(candidate,raw): continue
            seen.add(candidate.url); collected.append(candidate)
    brand,brand_support,official_domain=_select_brand(collected,raw,brand_hint or None)
    hint_key=_compact(brand_hint); selected_key=_compact(brand)
    ...
    if brand_hint and _plausible_brand_hint(brand_hint) and not aligned:
        brand=brand_hint; brand_support=0; official_domain=None
    ...
    if brand and (_compact(brand)!=_compact(current.brand) or not current.brand):
        updates["brand"]=brand
        if not current.manufacturer: updates["manufacturer"]=brand
    current_model=str(current.model or current.product_name or "").strip(); needs=bool(not current_model or _compact(current_model)==_compact(raw) or len(current_model)>70)
    stable_current=stable_model_core(current_model,raw=raw,brand=brand or brand_hint)
    if needs and stable_current:
        updates["model"]=stable_current; updates["product_name"]=(f"{brand or brand_hint} {stable_current}" if (brand or brand_hint) else stable_current).strip()
    if model and _compact(model)!=_compact(raw) and needs:
        selected_core=stable_model_core(model,raw=raw,brand=brand or brand_hint) or model
        if not all(t in _MODEL_DESCRIPTOR_STOP for t in _tokenize(selected_core)):
            updates["model"]=selected_core; updates["product_name"]=(f"{brand or brand_hint} {selected_core}" if (brand or brand_hint) else selected_core).strip()
