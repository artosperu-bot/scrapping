...
class App(BaseApp):
    ...
    def _build_media_tab(self):
        ...
        ttk.Label(
            self.media_tab,
            text="Multimedia por producto — proceso independiente del Excel",
            font=("Segoe UI", 11, "bold"),
        ).pack(anchor="w")
        ttk.Label(
            self.media_tab,
            text="URLs manuales primero → búsqueda web por Part Number/modelo → fabricante/oficial → extracción estructurada/API/HTML/Playwright → descarga validada.",
        ...
        ttk.Label(
            right,
            text="Una URL por línea. Se intenta antes de la búsqueda automática y siempre se valida contra el producto.",
        ).pack(anchor="w")
        ...
        ttk.Checkbutton(
            controls,
            text="Buscar también automáticamente por Part Number/modelo",
            variable=self.media_auto_search,
        ).pack(side="left")
        ttk.Button(controls, text="Guardar URLs", command=self._save_media_urls).pack(side="right")

        ttk.Label(
            right,
            text="La búsqueda automática prioriza páginas oficiales. En este módulo el color no bloquea si el modelo está validado.",
            font=("Segoe UI", 9, "italic"),
        ).pack(anchor="w", pady=(5, 0))

        action_row = ttk.Frame(self.media_tab)
        action_row.pack(fill="x", pady=(8, 5))
        self.media_selected_btn = ttk.Button(
            action_row,
            text="BUSCAR Y DESCARGAR MULTIMEDIA",
            command=self._run_media_selected,
        )
        self.media_selected_btn.pack(side="left")
        self.media_all_btn = ttk.Button(
            action_row,
            text="Procesar todos los productos",
            command=self._run_media_all,
        )
    ...
    def analyze_excel(self):
        if hasattr(self, "media_product_list"):
            self.media_product_list.delete(0, "end")
        self.media_manual_urls = {}
        self._media_current_index = None
        super().analyze_excel()
        if self.preflight is None or not hasattr(self, "media_product_list"):
            return
        ...
        for i, row in enumerate(self.product_rows):
            ...
            label = (
                (ident.mpn or ident.ean or ident.upc or ident.gtin or ident.model or ident.product_name)
                if ident
                else row.get("model") or row.get("product_name") or f"Producto {i + 1}"
            )
            self.media_product_list.insert("end", str(label))
        if self.product_rows:
            self.media_product_list.selection_set(0)
            self._on_media_product_select()
            self.media_status.set(f"{len(self.product_rows)} productos listos para multimedia.")
        self._clear_media_gallery()

    def _media_selected_index(self) -> int | None:
        sel = self.media_product_list.curselection()
    ...
    def _on_media_product_select(self, _event=None):
        ...
        if self._media_current_index is not None and self._media_current_index != new_index:
            self._capture_media_urls(self._media_current_index)
        if new_index is None:
            return
        self._media_current_index = new_index
        self.media_urls_text.delete("1.0", "end")
        self.media_urls_text.insert("1.0", "\n".join(self.media_manual_urls.get(new_index, [])))

    def _save_media_urls(self):
        index = self._media_selected_index()
        if index is None:
            messagebox.showwarning("Multimedia", "Selecciona primero un producto.")
            return
        self._capture_media_urls(index)
        self.media_status.set(f"URLs manuales guardadas: {len(self.media_manual_urls.get(index, []))}")
    ...
    def _run_media_selected(self):
        ...
        if index is None:
            messagebox.showwarning("Multimedia", "Selecciona primero un producto.")
            return
        self._save_media_urls()
        self._start_media_indices([index])

    def _run_media_all(self):
        if not self.product_rows:
            messagebox.showwarning("Multimedia", "Analiza primero un Excel con productos.")
            return
        self._capture_media_urls(self._media_selected_index())
        self._start_media_indices(list(range(len(self.product_rows))))
        ...
        def work():
            try:
                for pos, (index, identity) in enumerate(valid, 1):
                    ...
                    def on_event(event, product_index=index):
                        self.media_events.put({**event, "product_index": product_index})
    ...
    def _drain_media_events(self):
        try:
            while True:
                event = self.media_events.get_nowait()
                ...
                if event_type == "media":
                    ...
                    url = event.get("url")
                    self.media_status.set(f"{status}: {url}")
                    self.emit(f"[MEDIA] {status}: {url}")
                elif event_type == "error":
                    self.emit(f"[MEDIA] ERROR {event.get('url')}: {event.get('error')}")
                elif event_type == "done":
                    self.emit(
                        f"[MEDIA] producto terminado: {event.get('downloaded', 0)} descargados, "
                        f"{event.get('metadata_only', 0)} enlaces/video externo"
                    )
                elif event_type == "fatal":
                    self.emit(event.get("error") or "Error multimedia")
                    self.media_status.set("Error durante la búsqueda multimedia.")
                elif event_type == "batch_done":
                    self._media_running = False
                    self.media_selected_btn.configure(state="normal")
                    self.media_all_btn.configure(state="normal")
                    self.media_status.set(f"Terminado. {self._media_cards} elementos mostrados.")
        except queue.Empty:
            pass
        self.after(150, self._drain_media_events)
    ...
    def _clear_media_gallery(self):
        if not hasattr(self, "media_gallery"):
            return
        for child in self.media_gallery.winfo_children():
            child.destroy()
        self._media_photo_refs.clear()
        self._media_cards = 0

    def _add_media_card(self, item: dict, product_index: int | None):
        col_count = 5
        position = self._media_cards
        row, col = divmod(position, col_count)
        card = ttk.Frame(self.media_gallery, padding=5, relief="ridge")
        card.grid(row=row, column=col, padx=4, pady=4, sticky="nsew")
        self.media_gallery.columnconfigure(col, weight=1)
        ...
        if product_index is not None:
            ...
            if ident:
                product_label = str(ident.mpn or ident.model or ident.product_name or "")
        ...
        if media_type == "image" and local_path and Path(local_path).exists():
            try:
                ...
                self._media_photo_refs.append(photo)
                label = ttk.Label(card, image=photo)
                label.pack()
                rendered_image = True
            except Exception as exc:
                self.emit(f"[MEDIA] miniatura no disponible: {exc}")

        if not rendered_image:
            kind = "VIDEO" if media_type == "video" else "IMAGEN"
        ...
        if confidence is not None:
            ttk.Label(card, text=f"confianza: {confidence}", font=("Segoe UI", 8)).pack(anchor="w")

        def open_item(_event=None, path=local_path, url=source_url):
            self._open_media_item(path, url)

        for widget in (card, *card.winfo_children()):
            widget.bind("<Double-1>", open_item)
        self._media_cards += 1
    ...
    def _open_media_folder(self):
        path = Path(self.out.get()) / "multimedia"
        path.mkdir(parents=True, exist_ok=True)
        try:
            os.startfile(str(path))
        except Exception:
            messagebox.showinfo("Carpeta multimedia", str(path))
