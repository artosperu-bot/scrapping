...
class App(MediaProgressApp):
    ...
    def _build_price_tab(self):
        ...
        scroll = ttk.Scrollbar(box, orient="vertical", command=self.price_tree.yview); self.price_tree.configure(yscrollcommand=scroll.set)
        scroll.pack(side="right", fill="y"); self.price_tree.pack(side="left", fill="both", expand=True); self.price_tree.bind("<Double-1>", self._open_price_offer)

    def analyze_excel(self):
        if hasattr(self, "price_product_list"): self.price_product_list.delete(0, "end")
        super().analyze_excel()
        if not hasattr(self, "price_product_list"): return
        ...
        if self.product_rows:
            self.price_product_list.selection_set(0); self.price_status.set(f"{len(self.product_rows)} productos listos para comparar precios.")

    def _selected_price_index(self):
        sel = self.price_product_list.curselection(); return int(sel[0]) if sel else None

    def _run_price_selected(self):
        index = self._selected_price_index()
        if index is None: messagebox.showwarning("Precios", "Selecciona primero un producto."); return
        self._start_price_indices([index])

    def _run_price_all(self):
        if not self.product_rows: messagebox.showwarning("Precios", "Analiza primero un Excel con productos."); return
        self._start_price_indices(list(range(len(self.product_rows))))

    def _start_price_indices(self, indices):
        if self._price_running: messagebox.showinfo("Precios", "Ya hay una búsqueda de precios en ejecución."); return
        valid = [(i, self._identity_for_index(i)) for i in indices if self._identity_for_index(i) is not None]
        if not valid: messagebox.showerror("Precios", "No hay identidades válidas para procesar."); return
        output_root = self.out.get(); self._price_running = True; self._price_total = len(valid); self._price_completed = 0; self._price_had_error = False
        self.price_selected_btn.configure(state="disabled"); self.price_all_btn.configure(state="disabled")
        for item in self.price_tree.get_children(): self.price_tree.delete(item)
        self._set_price_progress(0, 0); self.price_progress_animation.set_running("Consultando precios…")
        def work():
            try:
                for pos, (index, identity) in enumerate(valid, 1):
                    ...
                    def on_event(event, product_index=index, product_label=label): self.price_events.put({**event, "product_index": product_index, "product_label": product_label})
    ...
    def _start_price_product_indicator(self):
        self.price_product_progress.stop(); self.price_product_progress.configure(mode="indeterminate"); self.price_product_progress.start(12); self.price_product_percent.set("En curso")
    ...
    def _set_price_progress(self, product_pct: int, overall_pct: int):
        ...
        product_pct = max(0, min(100, product_pct)); overall_pct = max(0, min(100, overall_pct))
        self.price_product_progress["value"] = product_pct; self.price_overall_progress["value"] = overall_pct
        self.price_product_percent.set(f"{int(product_pct)}%"); self.price_overall_percent.set(f"{int(overall_pct)}%")

    def _insert_price_offer(self, row: dict, label: str | None):
        price = row.get("selling_price"); list_price = row.get("list_price"); currency = str(row.get("currency") or "PEN")
    ...
    def _open_price_offer(self, _event=None):
        ...
        if values and len(values) >= 8 and values[7]: webbrowser.open(str(values[7]))
