...
def cross_field_issues(rec: ProductRecord, facts: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    """Validate normalized facts, not arbitrary page text.

    Raw parser noise has already been rejected/deduplicated by record_builder. A material
    contradiction is reported only when clean canonical evidence still disagrees.
    """
    ...
    for ev in rec.evidence:
        ...
        if n in {"no","false","0","not supported","unsupported"}:
            bt_false=True
        if re.search(r"\b(?:bluetooth\s*)?\d(?:\.\d)?\b",raw,re.I) and "version" in key_norm(ev.attribute):
            bt_version=True
...
def analyze_resolution(rec: ProductRecord, template_plan: dict | None) -> dict[str, Any]:
    ...
    for semantic in semantics:
        ...
        if is_conflict:
            ...
            if na:
                status=NOT_APPLICABLE; reason=na_reason
            elif _semantic_has_direct_evidence(rec,semantic):
                status=FOUND_DIRECT; reason="validated_evidence_matches_field_semantics"
            else:
                status=INSUFFICIENT_EVIDENCE; reason="no_validated_evidence_for_requested_semantic"
                research.append(semantic)
    ...
    for issue in issues:
        ...
        if term and term not in research: research.insert(0,term)
