...
class SocialVideoVisibilityMixin:
    ...
    ``media_desktop`` owns the real yt-dlp/FFmpeg download behavior. The organized
    presentation intentionally rebuilds the Multimedia tab, so this final-shell
    layer only restores the controls that were otherwise destroyed. No download
    ...
    def _build_media_tab(self):
        ...
        if children:
            # The organized UI currently ends with its Progress panel. Keep the
            # downloader immediately above it so the action stays visible.
        ...
        ttk.Label(
            social_box,
            textvariable=self.social_video_status,
