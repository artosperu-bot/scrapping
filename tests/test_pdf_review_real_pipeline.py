...
def test_real_review_shell_search_uses_general_identifier_pdf_service():
    source = Path("src/product_intelligence/real_pdf_review_shell.py").read_text(encoding="utf-8")
    assert "search_product_pdfs(" in source
    assert "search_product_pdfs_by_part_number" not in source
