...
def test_desktop_profile_includes_downloader_and_ffmpeg_runtime():
    project = (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    desktop = project.split('desktop = [', 1)[1].split(']', 1)[0]
    assert 'yt-dlp>=2026.7.4' in desktop
    assert 'imageio-ffmpeg>=0.6.0' in desktop


def test_pyinstaller_collects_yt_dlp_and_imageio_ffmpeg():
